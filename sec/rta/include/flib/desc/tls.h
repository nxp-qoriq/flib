...
/**
 * @file tls.h
 * @brief SSL/TLS/DTLS Shared Descriptor Constructors
...
#define DTLS_PDBOPTS_ARS128	0x80	/**< 128-entry DTLS anti-replay
					     window */
...
/**
 * @def TLS_PDBOPTS_OUTFMT_MASK
 * Output frame format mask (decapsulation only).
 */
#define TLS_PDBOPTS_OUTFMT_MASK	0x0C

/**
 * @def TLS_PDBOPTS_OUTFMT_FULL
 * Full copy of unencrypted fields from input frame to output frame.
 */
#define TLS_PDBOPTS_OUTFMT_FULL	0x08

/**
 * @def TLS_PDBOPTS_OUTFMT_RHP
 * Record header + payload; valid for SEC ERA >= 5.
 */
...
#define TLS_PDBOPTS_TR_ICV	0x10	/**< Available starting with
					     SEC ERA 5 */
...
/**
 * @def DTLS_DPOVRD_METADATA_LEN_SHIFT
 * Metadata length.
 */
#define DTLS_DPOVRD_METADATA_LEN_SHIFT	16

/**
 * @def DTLS_DPOVRD_METADATA_LEN_MASK
 * See DTLS_DPOVRD_METADATA_LEN_SHIFT.
 */
#define DTLS_DPOVRD_METADATA_LEN_MASK	(0xff << DTLS_DPOVRD_METADATA_LEN_SHIFT)

/**
 * @def TLS_DPOVRD_TYPE_MASK
 * Mask for TLS / DTLS type. Valid only for encapsulation.
 */
 ...
 * @ingroup tls_pdb
 * @details SSL3.0/TLS1.0/TLS1.1/TLS1.2 block encapsulation PDB part.
 */
#pragma pack(push, 1)
...
struct tls_block_enc {
	union {
		uint32_t word1;
		struct {
			uint8_t type; /**< protocol content type */
			uint8_t version[2]; /**< protocol version */
			uint8_t options; /**< PDB options */
		};
	};
	uint64_t seq_num; /**< protocol sequence number; big endian format */
};
#else
struct tls_block_enc {
	union {
 ...
 * @ingroup tls_pdb
 * @details DTLS1.0/DTLS1.2 block encapsulation PDB part
 */
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
struct dtls_block_enc {
	...
	union {
		struct {
			uint16_t epoch; /**< protocol epoch */
			uint16_t seq_num_hi; /**< protocol sequence number
						  (upper 16 bits) */
		};
		uint32_t word2;
	};
	uint32_t seq_num_lo; /**< protocol sequence number (lower 32 bits) */
};
#else
struct dtls_block_enc {
	union {
 ...
 * @ingroup tls_pdb
 * @details SSL3.0/TLS1.0/TLS1.1/TLS1.2 block decapsulation PDB part.
 */
#pragma pack(push, 1)
struct tls_block_dec {
	union {
		struct {
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
			uint8_t rsvd[3]; /**< reserved, do not use */
			uint8_t options; /**< PDB options */
#else
			uint8_t options; /**< PDB options */
			uint8_t rsvd[3]; /**< reserved, do not use */
...
/**
 * @struct dtls_block_dec tls.h
 * @ingroup tls_pdb
 * @details DTLS1.0/DTLS1.2 block decapsulation PDB part.
 */
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
 ...
 * @ingroup tls_pdb
 * @details SSL3.0/TLS1.0/TLS1.1/TLS1.2/DTLS1.0/DTLS1.2 block
...
struct tls_block_pdb {
	...
	uint8_t iv[16]; /**< initialization vector; for CBC-mode cipher suites,
			     the IV field is only 8 bytes if the PROTINFO field
			     of the Operation Command selects DES/3DES.  */
	uint32_t anti_replay[4]; /**< Anti-replay window - valid only for DTLS
				      decapsulation; size depends on
				      DTLS_PDBOPTS_ARS32/64/128 option flags;
				      big endian format */
	uint8_t icv_len; /**< ICV length; valid only if TLS_PDBOPTS_TR_ICV
			      option flag is set */
};
 ...
 * @ingroup tls_pdb
 * @details SSL3.0/TLS1.0/TLS1.1/TLS1.2 stream encapsulation PDB part.
 */
...
/**
 * @struct tls_stream_dec tls.h
 * @ingroup tls_pdb
 * @details SSL3.0/TLS1.0/TLS1.1/TLS1.2 stream decapsulation PDB part.
 */
 ...
 * @ingroup tls_pdb
 * @details SSL3.0/TLS1.0/TLS1.1/TLS1.2 stream encapsulation / decapsulation
 *          PDB.
 */
#pragma pack(push, 1)
struct tls_stream_pdb {
	...
	uint64_t seq_num; /**< protocol sequence number */
	uint8_t icv_len; /**< ICV length; valid only if TLS_PDBOPTS_TR_ICV
			      option flag is set */
};
#pragma pack(pop)
 ...
 * @ingroup tls_pdb
 * @details TLS1.1/TLS1.2 AES CTR encapsulation PDB part.
 */
#pragma pack(push, 1)
struct tls_ctr_enc {
	union {
 ...
 * @ingroup tls_pdb
 * @details PDB part for TLS1.1/TLS1.2 AES CTR decapsulation and DTLS1.0/DTLS1.2
 *          AES CTR encapsulation/decapsulation.
 */
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
struct tls_ctr {
	union {
 ...
 * @ingroup tls_pdb
 * @details TLS1.1/TLS1.2/DTLS1.0/DTLS1.2 AES CTR encapsulation / decapsulation
 *          PDB.
 *          TLS1.1/TLS1.2/DTLS1.0/DTLS1.2 AES CTR encryption processing is
 *          supported starting with SEC ERA 5.
 */
struct tls_ctr_pdb {
	union {
	...
	uint32_t write_iv_hi; /**< server write IV / client write IV
				   (upper 32 bits) */
	union {
		struct {
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
			uint16_t write_iv_lo; /**< server write IV / client
						   write IV (lower 32 bits) */
			uint16_t constant; /**< constant equal to 0x0000 */
#else
			uint16_t constant; /**< constant equal to 0x0000 */
			uint16_t write_iv_lo; /**< server write IV / client
						   write IV (lower 32 bits) */
 ...
 * @ingroup tls_pdb
 * @details TLS1.2 AES GCM encapsulation PDB part.
 */
#pragma pack(push, 1)
 ...
 * @ingroup tls_pdb
 * @details TLS1.2 AES GCM decapsulation PDB part.
 */
#pragma pack(push, 1)
...
/**
 * @struct dtls_gcm_enc tls.h
 * @ingroup tls_pdb
 * @details DTLS1.2 AES GCM encapsulation PDB part.
 */
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
struct dtls_gcm_enc {
	union {
...
/**
 * @struct dtls_gcm_dec tls.h
 * @ingroup tls_pdb
 * @details DTLS1.2 AES GCM decapsulation PDB part.
 */
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
struct dtls_gcm_dec {
	union {
 ...
 * @ingroup tls_pdb
 * @details TLS1.2/DTLS1.2 AES GCM encapsulation / decapsulation PDB.
 */
struct tls_gcm_pdb {
	union {
	...
	uint8_t salt[4]; /**< 4-byte array salt */
	uint32_t anti_replay[4]; /**< Anti-replay window - valid only for DTLS
				      decapsulation; size depends on
				      DTLS_PDBOPTS_ARS32/64/128 option flags;
				      big endian format */
 ...
 * @ingroup tls_pdb
 * @details TLS1.2 AES CCM encapsulation PDB part.
 */
#pragma pack(push, 1)
 ...
 * @ingroup tls_pdb
 * @details PDB part for TLS12 AES CCM decapsulation PDB and DTLS1.2 AES CCM
 ...
 * @ingroup tls_pdb
 * @details TLS1.2/DTLS1.2 AES CCM encapsulation / decapsulation PDB.
 */
#if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
struct tls_ccm_pdb {
	union {
	...
	uint32_t write_iv; /**< server write IV / client write IV */
	union {
		struct {
			uint8_t b0_flags; /**< use 0x5A for 8-byte ICV, 0x7A for
					       16-byte ICV */
			uint8_t ctr0_flags; /**< equal to 0x2 */
			uint8_t rsvd1[2]; /**< reserved, do not use */
		};
		uint32_t word1;
	};
	union {
		struct {
			uint8_t rsvd2; /**< reserved, do not use */
			uint8_t ctr0[3]; /**< CR0 lower 3 bytes, set to 0 */
		};
		uint32_t word2;
	};
	uint32_t anti_replay[4]; /**< Anti-replay window - valid only for DTLS
...
/**
 * @enum tls_cipher_mode tls.h
 * @details (D)TLS cipher mode
 */
...
/**
 * @details Get DTLS anti-replay scorecard size.
 * @param[in] options 1st word in the DTLS PDB
...
/**
 * @details Get TLS cipher mode based on IANA cipher suite value
 * @param[in] protinfo protocol information
 ...
 *            construction. No error checking will be made. Refer to the block
 *            guide for details of the PDB.
...
/**
 * @def CWAP_DTLS_ENC_BASE_SD_LEN
 * CAPWAP/DTLS encapsulation shared descriptor length.
...
/**
 * @def CWAP_DTLS_DEC_BASE_SD_LEN
 * CAPWAP/DTLS decapsulation shared descriptor length.
 * Accounts only for the "base" commands, i.e. excludes KEY / DKP commands,
 * immediate keys, and is intended to be used by upper layers to determine
 * whether keys can be inlined or not.
 * To be used as first parameter of rta_inline_query(), added with PDB length.
...
/**
 * @details DTLS (in CAPWAP context) block cipher encapsulation / decapsulation
 *          shared descriptor.
 * @ingroup sharedesc_group
 * @param[in,out] descbuf pointer to buffer used for descriptor construction
 * @param[in] ps if 36/40bit addressing is desired, this parameter must be true
