 ...
 * supported.
 */
static const unsigned move_src_table_sz[] = {9, 11, 14, 14, 14, 14};
 ...
 * supported.
 */
static const unsigned move_dst_table_sz[] = {13, 14, 14, 15, 15, 15};
				...
				uint32_t flags)
{
	...
	if (type_length == REG_TYPE) {
		...
		if ((length != _MATH0) && (length != _MATH1) &&
			...
			pr_debug("MOVE: MOVE_LEN length must be MATH[0-3]. SEC PC: %d; Instr: %d\n",
				 program->current_pc,
				 program->current_instruction);
			goto err;
		}
				  ...
				  uint16_t *opt)
{
		...
		if (dst == _DESCBUF) {
			...
			*offset = dst_offset;
			...
			if ((src_offset) && (dst_offset)) {
				pr_debug("MOVE: Bad offset. SEC PC: %d; Instr: %d\n",
					 program->current_pc,
					 program->current_instruction);
				goto err;
			}
	...
	case (_OFIFO):
		...
		if (((dst == _IFIFOAB1) || (dst == _IFIFOAB2) ||
			...
			pr_debug("MOVE: Offset should be zero. SEC PC: %d; Instr: %d\n",
				 program->current_pc,
				 program->current_instruction);
			goto err;
		}
