			 ...
			 uint32_t mdatalen, uint32_t cipher_alg)
{
	...
	SHR_HDR(SHR_SERIAL, 7, 0);
	{
		...
		pmove3 =
		    MOVE(DESCBUF, previous_iv, MATH0, 0, IMM(16),
			 WITH(WAITCOMP));
		 ...
		 * jump to */
		pjump2 = JUMP(IMM(new_IV_OK), LOCAL_JUMP, ALL_FALSE,
				WITH(MATH_Z));
