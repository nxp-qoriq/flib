					   ...
					   unsigned buffpos)
{
	...
	SHR_HDR(SHR_NEVER, 6, 0);
	{
		 ...
		 *                              srcoffs=0(w0) wait */
		MOVE(DESCBUF, 0, MATH2, 0, 0, WAITCOMP | IMMED);
		...
		MATHB(MATH0, ADD, 0x78820000, MATH0, 4, NFU | IMMED2);
		...
		pmove1 = MOVE(MATH0, 0, DESCBUF, 0, 4, WAITCOMP | IMMED);
		 ...
		 *                              offset=22 local->[56] */
		ref_jump_reload = JUMP(reload, LOCAL_JUMP, ALL_TRUE, MATH_N);
		SET_LABEL(lab35);
		 ...
		 *                              dstoffs=40 */
		MOVE(MATH1, 0, CONTEXT2, 40, 8, IMMED);
		/* [44] A00104F7         jump:  jsl0 all-mismatch[math-z]
		 *                              offset=-9 local->[35] */
		pjump3 = JUMP(lab35, LOCAL_JUMP, ANY_FALSE, MATH_Z);
		 ...
		 *                              offset=-28 local->[23] */
		pjump2 = JUMP(lab23, LOCAL_JUMP, ALL_TRUE, 0);
		SET_LABEL(do_dcrc);
					   ...
					   unsigned buffpos)
{
	...
	PROGRAM_CNTXT_INIT(buff, buffpos);
	PROGRAM_SET_36BIT_ADDR();
	JOB_HDR(SHR_ALWAYS, buffpos, shraddr, REO | SHR);
	{
		SET_LABEL(reload);
		SEQINPTR(0, 0, 0);
		SEQOUTPTR(0, 0, 0);
