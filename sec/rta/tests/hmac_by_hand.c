/* Copyright 2008-2013 Freescale Semiconductor, Inc. */

#include <stdio.h>
#include "flib/rta.h"
#include "test_common.h"

enum rta_sec_era rta_sec_era;

uint8_t *iv1 = (uint8_t *)0x64;	/* input constant */
uint8_t *ipad = (uint8_t *)0x664;
uint8_t *opad = (uint8_t *)0x12364;

unsigned build_hmacprecomp(uint32_t *buff)
{
	struct program prg;
	struct program *program = &prg;
	uint8_t *hmac_key = (uint8_t *)(0x12);	/* input */
	unsigned int hmac_key_len = 18;	/* input */
	uint32_t *hmacprecompute_opad_phys = (uint32_t *)0xaabb2200;

	PROGRAM_CNTXT_INIT(buff, 0);
	JOB_HDR(SHR_NEVER, 0, 0, 0);
	{
		LOAD((uintptr_t) hmac_key, CONTEXT1, 0, hmac_key_len, 0);
		MATHB(ZERO, XOR, 0x3636363636363636, MATH0, 8, IMMED2);
		JUMP(1, LOCAL_JUMP, ALL_TRUE, NCP);

		/* Apply the IPAD to the key */
		MOVE(CONTEXT1, 0, MATH1, 0, 24, WAITCOMP | IMMED);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 0, 24, WAITCOMP | IMMED);

		MOVE(CONTEXT1, 24, MATH1, 0, 24, WAITCOMP | IMMED);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 24, 24, WAITCOMP | IMMED);

		MOVE(CONTEXT1, 48, MATH1, 0, 24, WAITCOMP | IMMED);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 48, 24, WAITCOMP | IMMED);

		MOVE(CONTEXT1, 72, MATH1, 0, 24, WAITCOMP | IMMED);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 72, 24, WAITCOMP | IMMED);

		/* Init hash IV appropriate to desired algorithm */
		LOAD((uintptr_t) iv1, CONTEXT2, 0, 64, 0);

		/* Process padded key as first half of first block of data */
		MOVE(CONTEXT1, 0, IFIFOAB2, 0, 64, IMMED);
		ALG_OPERATION(OP_ALG_ALGSEL_SHA512, OP_ALG_AAI_HASH,
			      OP_ALG_AS_UPDATE, 0, 0);

		/* Send IPAD as rest of block */
		MATHB(ZERO, XOR, MATH0, MATH1, 8, 0);
		MATHB(ZERO, XOR, MATH0, MATH2, 8, 0);
		MATHB(ZERO, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH0, 0, IFIFOAB2, 0, 32, IMMED);
		MOVE(MATH0, 0, IFIFOAB2, 0, 32, LAST2 | IMMED);

		/* This happens when MDHA is finished */
		STORE(CONTEXT2, 0, (uintptr_t) ipad, 64, 0);
		JUMP((uintptr_t) hmacprecompute_opad_phys, FAR_JUMP, ALL_TRUE,
		     0);
	}

	return PROGRAM_FINALIZE();
}

unsigned build_hmacprecomp_opad(uint32_t *buff)
{
	struct program prg;
	struct program *program = &prg;

	PROGRAM_CNTXT_INIT(buff, 0);
	JOB_HDR(SHR_NEVER, 0, 0, 0);
	{
		LOAD(CLRW_CLR_C2CTX | CLRW_CLR_C2MODE | CLRW_CLR_C2DATAS,
		     CLRW, 0, 4, IMMED);
		LOAD(CCTRL_RESET_CHA_MDHA, CCTRL, 0, 4, IMMED);

		/* Apply OPAD^IPAD to the key */
		MATHB(MATH1, XOR, 0x5C5C5C5C5C5C5C5C, MATH1, 8, IMMED2);

		MOVE(CONTEXT1, 0, MATH1, 0, 24, WAITCOMP | IMMED);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 0, 24, WAITCOMP | IMMED);

		MOVE(CONTEXT1, 24, MATH1, 0, 24, WAITCOMP | IMMED);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 24, 24, WAITCOMP | IMMED);

		MOVE(CONTEXT1, 48, MATH1, 0, 24, WAITCOMP | IMMED);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 48, 24, WAITCOMP | IMMED);

		MOVE(CONTEXT1, 72, MATH1, 0, 24, WAITCOMP | IMMED);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 72, 24, WAITCOMP | IMMED);

		/* Process padded key as first block of data */
		LOAD((uintptr_t) iv1, CONTEXT2, 0, 64, 0);
		MOVE(CONTEXT1, 0, IFIFOAB2, 0, 64, IMMED);
		ALG_OPERATION(OP_ALG_ALGSEL_SHA512, OP_ALG_AAI_HASH,
			      OP_ALG_AS_UPDATE, 0, 0);

		/* Send OPAD as rest of block */
		MATHB(ZERO, XOR, 0x5C5C5C5C5C5C5C5C, MATH0, 8, IMMED2);
		MATHB(ZERO, XOR, MATH0, MATH1, 8, 0);
		MATHB(ZERO, ADD, MATH0, MATH2, 8, 0);
		MATHB(ZERO, OR, MATH0, MATH3, 8, 0);
		MOVE(MATH0, 0, IFIFOAB2, 0, 32, IMMED);
		MOVE(MATH0, 0, IFIFOAB2, 0, 32, IMMED);
		MOVE(MATH0, 0, IFIFOAB2, 0, 32, LAST2 | IMMED);

		/* This happens when MDHA is finished */
		STORE(CONTEXT2, 0, (uintptr_t) opad, 0, 0);
	}

	return PROGRAM_FINALIZE();
}

unsigned build_hmac_from_pre(uint32_t *buff)
{
	struct program prg;
	struct program *program = &prg;
	uint8_t *innerhash = (uint8_t *)0x28;	/* debug */
	uint8_t *mac = (uint8_t *)12228;
	void *msg = (void *)0x12341234ul;	/* physical ptr */
	unsigned int maclen = 28;

	PROGRAM_CNTXT_INIT(buff, 0);
	JOB_HDR(SHR_NEVER, 0, 0, 0);
	{
		/* Set up current state of inner hash */
		LOAD((uintptr_t) ipad, CONTEXT2, 0, 64, 0);
		JUMP(1, LOCAL_JUMP, ALL_TRUE, NIP);
		LOAD(0x0000000000000080, CONTEXT2, 64, 8, IMMED);

		FIFOLOAD(MSG2, (uintptr_t) msg, 0, LAST2);
		ALG_OPERATION(OP_ALG_ALGSEL_SHA512, OP_ALG_AAI_HASH,
			      OP_ALG_AS_FINALIZE, 0, 0);
		JUMP(1, LOCAL_JUMP, ALL_TRUE, CLASS2);

		LOAD(CLRW_CLR_C2MODE | CLRW_CLR_C2DATAS, CLRW, 0, 4, IMMED);
		LOAD(CCTRL_RESET_CHA_MDHA, CCTRL, 0, 4, IMMED);

		STORE(CONTEXT2, 0, (uintptr_t) innerhash, maclen, 0);
		JUMP(1, LOCAL_JUMP, ALL_TRUE, NOP);

		/* Make inner hash be the message for the outer hash */
		MOVE(CONTEXT2, 0, IFIFOAB2, 0, maclen, LAST2 | IMMED);
		/* Set up current state of outer hash */
		LOAD((uintptr_t) opad, CONTEXT2, 0, 64, 0);
		JUMP(1, LOCAL_JUMP, ALL_TRUE, NIP);

		LOAD(0x0000000000000080, CONTEXT2, 64, 8, IMMED);
		ALG_OPERATION(OP_ALG_ALGSEL_SHA512, OP_ALG_AAI_HASH,
			      OP_ALG_AS_FINALIZE, 0, 0);
		STORE(CONTEXT2, 0, (uintptr_t) mac, maclen, 0);
	}

	return PROGRAM_FINALIZE();
}

uint32_t prg_buff[1000];

int main(int argc, char **argv)
{
	unsigned size;

	rta_set_sec_era(RTA_SEC_ERA_1);

	pr_debug("HMAC_By_Hand #1 JD program\n");
	size = build_hmacprecomp(prg_buff);
	pr_debug("size = %d\n", size);
	print_prog(prg_buff, size);

	pr_debug("HMAC_By_Hand #2 JD program\n");
	size = build_hmacprecomp_opad(prg_buff);
	pr_debug("size = %d\n", size);
	print_prog(prg_buff, size);

	pr_debug("HMAC_By_Hand #3 JD program\n");
	size = build_hmac_from_pre(prg_buff);
	pr_debug("size = %d\n", size);
	print_prog(prg_buff, size);

	return 0;
}
