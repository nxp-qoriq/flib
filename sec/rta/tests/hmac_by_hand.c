...
unsigned build_hmacprecomp(uint32_t *buff)
{
	...
	JOB_HDR(SHR_NEVER, 0, 0, 0);
	{
		...
		JUMP(IMM(1), LOCAL_JUMP, ALL_TRUE, NCP);

		/* Apply the IPAD to the key */
		MOVE(CONTEXT1, 0, MATH1, 0, IMM(24), WAITCOMP);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 0, IMM(24), WAITCOMP);

		MOVE(CONTEXT1, 24, MATH1, 0, IMM(24), WAITCOMP);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 24, IMM(24), WAITCOMP);

		MOVE(CONTEXT1, 48, MATH1, 0, IMM(24), WAITCOMP);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 48, IMM(24), WAITCOMP);

		MOVE(CONTEXT1, 72, MATH1, 0, IMM(24), WAITCOMP);
		MATHB(MATH1, XOR, MATH0, MATH1, 8, 0);
		MATHB(MATH2, XOR, MATH0, MATH2, 8, 0);
		MATHB(MATH3, XOR, MATH0, MATH3, 8, 0);
		MOVE(MATH1, 0, CONTEXT1, 72, IMM(24), WAITCOMP);
		...
		MOVE(MATH0, 0, IFIFOAB2, 0, IMM(32), LAST2);
...
unsigned build_hmac_from_pre(uint32_t *buff)
{
	...
	JOB_HDR(SHR_NEVER, 0, 0, 0);
	{
		...
		FIFOLOAD(MSG2, PTR((uintptr_t) msg), 0, LAST2);
		ALG_OPERATION(OP_ALG_ALGSEL_SHA512, OP_ALG_AAI_HASH,
		...
		JUMP(IMM(1), LOCAL_JUMP, ALL_TRUE, NOP);

		/* Make inner hash be the message for the outer hash */
		MOVE(CONTEXT2, 0, IFIFOAB2, 0, IMM(maclen), LAST2);
