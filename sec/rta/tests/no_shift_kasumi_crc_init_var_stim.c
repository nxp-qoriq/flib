...
int main(int argc, char *argv[])
{
	...
	/* Patch section start */
	PATCH_MOVE_NON_LOCAL(&more_extra_prgm, encap_share_end_ref3, &lte_prgm,
			     encap_share_end);

	PATCH_JUMP_NON_LOCAL(&more_extra_prgm, encap_share_end_ref4, &lte_prgm,
			     encap_share_end);

	PATCH_MOVE_NON_LOCAL(&even_more_extra_prgm, encap_share_end_ref5,
			     &lte_prgm, encap_share_end);
	...
	PATCH_JUMP_NON_LOCAL(&still_more_extra_prgm, back_from_extras_ref1,
			     &lte_prgm, back_from_extras);

	PATCH_MOVE_NON_LOCAL(&even_more_extra_prgm, yet_more_ref1,
			     &still_more_extra_prgm, yet_more);

	PATCH_JUMP_NON_LOCAL(&extra_prgm, loop_start_ref1, &lte_prgm,
			     loop_start);
	/* Patch section end */
