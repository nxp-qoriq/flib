...
unsigned test_perf(uint32_t *buff)
{
...

	{
		...
		MATHB(MATH0, ADD, IMM(0x0840010008880000), MATH3, 8, 0);
		MATHB(IMM(0x08400100009990), XOR, MATH1, MATH3, 8, 0);
		SET_LABEL(test1);
		MATHB(IMM(0x0840010000aaa0), XOR, MATH1, MATH3, 8, 0);
		SET_LABEL(test2);
		...
		WORD(seqnum);	/* Lower bits of SeqNum */
		JUMP(IMM(test1), LOCAL_JUMP, ALL_TRUE, 0);
		MATHB(MATH0, XOR, IMM(0x0840010008880000), MATH3, 8, 0);
		MATHB(IMM(0x08400100009990), XOR, MATH1, MATH3, 8, 0);
		MATHB(IMM(0x0840010000aaa0), XOR, MATH1, MATH3, 8, 0);
		MATHB(MATH2, XOR, MATH1, MATH3, 4, 0);
		MATHU(MATH2, BSWAP, MATH3, 2, NFU);
		...
		MOVE(DESCBUF, seqoutptr, MATH0, 0, IMM(16), WAITCOMP);
