...
unsigned test_fifo_load_op(uint32_t *buff)
{
	...
	FIFOLOAD(MSG1, PTR(msg), 1, FLUSH1);
	FIFOLOAD(MSG1, PTR(msg), 2, LAST1);
	FIFOLOAD(MSG2, PTR(msg), 3, LAST2);
	FIFOLOAD(MSG2, PTR(msg), 4, 0);

	FIFOLOAD(BIT_DATA, PTR(msg), 4003, CLASS1 | FLUSH1);
	FIFOLOAD(BIT_DATA, PTR(msg), 4003, CLASS2 | LAST2 | EXT);

	FIFOLOAD(MSGOUTSNOOP, PTR(src_dma), 0x212, LAST1 | LAST2 | SGF);
	FIFOLOAD(MSGINSNOOP, PTR(src_dma), 0x212, LAST1 | LAST2 | SGF);
	FIFOLOAD(AAD1, PTR(aad_ptr), 800, SGF);
	FIFOLOAD(AAD1, PTR(aad_ptr), 800, 0);
	FIFOLOAD(ICV1, PTR(icv_ptr), 12, LAST1);
	FIFOLOAD(ICV1, PTR(icv_ptr), 12, FLUSH1 | LAST2 | BOTH | EXT);
	FIFOLOAD(ICV1, PTR(icv_ptr), 12, FLUSH1);
	FIFOLOAD(ICV2, PTR(icv_ptr), 12, LAST2);
	FIFOLOAD(ICV2, PTR(icv_ptr), 12, LAST2);
	FIFOLOAD(IV1, PTR(iv_ptr), 12, LAST1);
	FIFOLOAD(IV1, PTR(iv_ptr), 12, LAST1);

	FIFOLOAD(MSGINSNOOP, IMM(0x616263), 3, LAST1 | LAST2);
